...
import { DemandeCv } from '../models/DemandeCv';
...
export class UserService {
  ...
  private URL4 = 'http://localhost:8075/DemandeCv'
  ...
  sendMail(to: string, subject: string, body: string) {
    ...
    return this.http.post('http://localhost:8075/mail/send-from-TYM', formData);
  }
  ...
  getCvById(id: number): Observable<DemandeCv> {
    const url = `${this.URL4}/getCvBy/${id}`;
    return this.http.get<DemandeCv>(url);
